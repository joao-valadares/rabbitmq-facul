...
def main():
    ...
    print_scenario_header(
        SCENARIO_NAME, 
        COMPONENT_NAME, 
        "Worker 2 - Processa tarefas da fila compartilhada em round-robin"
    )
    ...
    def callback(ch, method, properties, body):
        ...
        try:
            ...
            # Simula o processamento da tarefa (Worker 2 é um pouco mais rápido)
            logger.info(f"[{CONSUMER_ID}] 🚀 Iniciando processamento otimizado...")
            for i in range(estimated_time):
                time.sleep(0.9)  # 10% mais rápido que Worker 1
                logger.info(f"[{CONSUMER_ID}] 📊 Progresso: {i+1}/{estimated_time}s")
            
            logger.info(f"[{CONSUMER_ID}] ✅ Tarefa #{task_id} concluída rapidamente!")
