...
def main():
    ...
    print_scenario_header(
        SCENARIO_NAME, 
        COMPONENT_NAME, 
        "Worker 3 - Processa tarefas da fila compartilhada em round-robin"
    )
    ...
    def callback(ch, method, properties, body):
        ...
        try:
            ...
            # Simula o processamento da tarefa (Worker 3 é mais detalhado)
            logger.info(f"[{CONSUMER_ID}] 🚀 Iniciando processamento detalhado...")
            for i in range(estimated_time):
                time.sleep(1.1)  # 10% mais lento mas mais detalhado
                progress = ((i + 1) / estimated_time) * 100
                logger.info(f"[{CONSUMER_ID}] 📊 Progresso detalhado: {progress:.1f}% ({i+1}/{estimated_time}s)")
            
            logger.info(f"[{CONSUMER_ID}] ✅ Tarefa #{task_id} concluída com análise completa!")
